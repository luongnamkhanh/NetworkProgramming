        ...
        for (i = 0; i < MAX_CLIENTS; i++)
        {
            if (client_fds[i] != -1 && FD_ISSET(client_fds[i], &read_fds))
            {
                ...
                else if (state[i] == 1)
                {
                    ...
                    {
                        strcpy(recv_data, "LOGIN_SUCCESS\n");
                        bytes_sent = send(client_fds[i], recv_data, strlen(recv_data), 0);
                        if (bytes_sent < 0)
                        {
                            perror("\nError4: ");
                ...
                else if (state[i] == 2)
                {
                    ...
                    {
                        strcpy(recv_data, "LOGOUT_SUCCESS\n");
                        bytes_sent = send(client_fds[i], recv_data, strlen(recv_data), 0);
                        if (bytes_sent < 0)
                        {
                            perror("\nError8: ");
                    ...
                    else if (strcmp(recv_data, "2") == 0)
                    {
                        ...
                        char image_buffer[BUFF_SIZE];
                        int bytes_read;

                        image_file = fopen(image_filename, "r");
                        if (image_file == NULL)
                        {
                            strcpy(recv_data, "Error opening image file\n");
                            bytes_sent = send(client_fds[i], recv_data, strlen(recv_data), 0);
                            if (bytes_sent < 0)
                            {
                                perror("\nError9: ");
                        ...
                        else
                        {
                            ...
                            {
                                bytes_sent = send(client_fds[i], image_buffer, bytes_read, 0);
                                if (bytes_sent < 0)
                                {
                                    perror("\nError10: ");
                                    return 0;
